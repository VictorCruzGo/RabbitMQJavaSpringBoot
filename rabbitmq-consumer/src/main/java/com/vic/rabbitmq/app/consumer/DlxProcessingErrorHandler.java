...
import com.vic.rabbitmq.app.rabbitmq.RabbitmqHeader;
...
/*
 * Esta clase sera usada en la en el bloque Catch del consumer
 */
...
public class DlxProcessingErrorHandler {
	 ...
	 */
	//Recibe como parametro el nombre del exchange de los mensajes no procesados (DEAD)
	...
	//Recibe como parametro el nombre del exchange de los mensajes no procesados (DEAD) y la opcion numero de reintentos
	...
	//Comprueba a cantidad de reintento del mensaje
	public boolean handleErrorProcessingMessage(Message message, Channel channel) {
		...
		try {
			if (rabbitMqHeader.getFailedRetryCount() >= maxRetryCount) {
				...
				//log.warn("[DEAD] Error at " + new Date() + " on retry " + rabbitMqHeader.getFailedRetryCount() + " for message " + message);
				System.out.println("DLX WARM--[DEAD] Error at " + new Date() + " on retry " + rabbitMqHeader.getFailedRetryCount()	+ " for message " + message);
				
				//Cuando la cantidad de reintentos del mensaje es mayor a 3, mueve el mensaje a DeadExchange
				//Rabbitmq no tiene la opcion de mover mensajes. Para mover hay que seguir 2 pasos: publicar y confirmar el proceso.
				...
				//log.debug("[REQUEUE] Error at " + new Date() + " on retry " + rabbitMqHeader.getFailedRetryCount()+ " for message " + message);
				System.out.println("DLX DEBUG--[REQUEUE] Error at " + new Date() + " on retry " + rabbitMqHeader.getFailedRetryCount()+ " for message " + message);
				//El mensaje es enviado a exchange WAIT
				channel.basicReject(message.getMessageProperties().getDeliveryTag(), false);
			}
			return true;
		} catch (IOException e) {
			//log.warn("[HANDLER-FAILED] Error at " + new Date() + " on retry " + rabbitMqHeader.getFailedRetryCount()+ " for message " + message);
			System.out.println("DLX WARM--[HANDLER-FAILED] Error at " + new Date() + " on retry " + rabbitMqHeader.getFailedRetryCount()+ " for message " + message);
		}
