	...
	public boolean handleErrorProcessingMessage(Message message, Channel channel) {
		...
		try {
			if (rabbitMqHeader.getFailedRetryCount() >= maxRetryCount) {
				...
				System.out.println("CONSUMER - WARN: DeadExchangeName "+getDeadExchangeName()
				+" RoutingKey "+getRoutingKey()+"  [DEAD] Error at " + new Date() 
				+ " on retry " + rabbitMqHeader.getFailedRetryCount()+ " for message " + message);
			
				channel.basicPublish(getDeadExchangeName(), getRoutingKey(), null, message.getBody());
				channel.basicAck(message.getMessageProperties().getDeliveryTag(), false);
			} else {
				System.out.println("CONSUMER DEBUG: [REQUEUE] Error at " + new Date() 
						+ " on retry " + rabbitMqHeader.getFailedRetryCount()	+ " for message " + message);
			...
			System.out.println("EXCEPTION WARM: [HANDLER-FAILED] Error at " + new Date() + " on retry " + rabbitMqHeader.getFailedRetryCount()
					+ " for message " + message);
