	...
	public void listen(Message message, Channel channel) throws InterruptedException, JsonParseException, JsonMappingException, IOException {
		try {
			...
			if (StringUtils.isEmpty(emp.getName())) {
				...
				System.out.println("ACCOUNTING CONSUMER: On accounting :"+ emp);
				//log.info("On accounting : {}", emp);
				channel.basicAck(message.getMessageProperties().getDeliveryTag(), false);
			}
		} catch (Exception e) {
			System.out.println("EXCEPTION ACCOUNTIN CONSUMER: Error processing message : "+new String(message.getBody())+" : "+e.getMessage());
