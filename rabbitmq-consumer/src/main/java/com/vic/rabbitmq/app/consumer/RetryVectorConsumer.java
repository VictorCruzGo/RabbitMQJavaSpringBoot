	...
	public void listen(Message message, Channel channel)
		...
		try {
			...
			if (p.getSize() > 9000) {
				...
				System.out.println("COSUMER VECTOR EXCEPTION - Size too large");
				throw new IOException("Size too large");
			} else {
				//log.info("Convert to image, creating thumbnail, & publishing : " + p);
				System.out.println("CONSUMER VECTOR INFO-Convert to image, creating thumbnail, & publishing : " + p);
			...
			System.out.println("CONSUMER VECTOR WARM-Error processing message : " + new String(message.getBody()) + " : " + e.getMessage());
